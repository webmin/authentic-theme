    ...
    && length $in{'authenticated_logo_file'} )
{
    ...
    write_file_contents( $config_directory . $lr,
        $in{'authenticated_logo_file'} );
    if ( usermin_available() ) {
        unlink_file( $_usermin_config_directory . $lr );
        write_file_contents(
            $_usermin_config_directory . $lr,
            $in{'authenticated_logo_file'} );

        unlink_file(
            $_usermin_root_directory . $lrd );
        write_file_contents(
            $_usermin_root_directory . $lrd,
            $in{'authenticated_logo_file'} );
    }
    ...
    && length $in{'unauthenticated_logo_file'} )
{
    ...
    if ( usermin_available() ) {
        unlink_file( $_usermin_config_directory
                . $lw );
        write_file_contents(
            $_usermin_config_directory . $lw,
            $in{'unauthenticated_logo_file'}
        );

        unlink_file( $_usermin_root_directory
                . $lwd );
        write_file_contents(
            $_usermin_root_directory
                . $lwd,
            $in{'unauthenticated_logo_file'}
        );
    }
