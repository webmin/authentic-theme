...
sub theme_ui_form_end
{
    ...
    if (!$nojs) {
        ...
        $rv .= "<script type='application/javascript'>\n";
...
"<span><input class='ui_opt_textbox form-control' style='display: inline; width: auto; height: 28px; padding-top: 0; padding-bottom: 2px;$min_width' type='$type' name=\""
    ...
    if (defined($extra)) {
        $rv .= "<script type='application/javascript'>\$('#content form input[name=\"${name}_def\"]:checked').click();</script>";
    }
    return $rv;
}
...
sub theme_header_redirect_download
{
    ...
    my $script =
      ...
      $url . '" method="post" name="redirect"></form><script type="application/javascript">setTimeout(function(){document.forms.redirect.submit()}, ' .
