...
if ( !$in{'password'} ) {
  if ( index( $text{'session_postfix'}, "href" ) != '-1' ) {
      ...
      print
        '<button onclick=\'window.open("'. $link->[0] . '", "'. $link->[1] .'", "toolbar=no,menubar=no,scrollbars=no,resizable=yes,width=700,height=500");return false;\' class="btn btn-warning"><i class="fa fa-unlock"></i>&nbsp;&nbsp;'
        . &Atext('login_reset')
        . '</button>' . "\n";
  }
    ...
    print
'<button class="btn btn-default" type="submit" name="restart"><i class="fa fa-backup fa-1_25x"></i>&nbsp;&nbsp;'
      . &Atext('pam_restart')
      . '</button>' . "\n";
}
print
