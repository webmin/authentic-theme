            ...
            success: function(e) {
                $.each(e, function(k, n) {
                    ...
                    setTimeout(function() {
                        ...
                        if (k == "local_time" && settings_window_replace_timestamps) {
                            var o = $('#right-side-tabs .info-container .info-list-data span[data-data="' + k + '"] a');
                            if (o && o.length) {
                                o.html(o.next("span").detach())
                            }
                            var q = t__wi_p.$(".info-container").find("span[data-convertible-timestamp-full]"),
                                s = q.attr("data-convertible-timestamp-full");
                            q.html(moment.unix(s).format(settings_window_replaced_timestamp_format_full))
                        }
                        var r = {};
                        $(".badge-drivestatus").each(function(A, x) {
                            var w = parseInt($(this).text().split(":")[1]),
                                y = (w <= 30 ? "--bg-info" : (w > 30 && w <= 60 ? "--bg-success" : (w > 60 && w <= 80) ? "bg-warning" : (w > 80) ? "bg-danger" : "")),
                                z = $(this).text().indexOf("°C") > -1;
                            if (!z) {
                                y = (w <= 86 ? "--bg-info" : (w > 86 && w <= 140 ? "--bg-success" : (w > 140 && w <= 176) ? "bg-warning" : (w > 176) ? "bg-danger" : ""))
                            }
                            $(this).addClass(y);
                            if (y == "bg-warning" || y == "bg-danger") {
                                r[$(x).attr("data-stats")] = 1
                            }
                        ...
                        if (k == "warning_si") {
                            ...
                            p.html(n.replace(/<(script|link|meta)\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/(script|link|meta)>/gi, "").replace("type='submit'", "formtarget='page' type='submit'").replace("class='ui_submit", "class='ui_submit btn btn-default btn-xs"));
                            ((n == "<br>") && p.empty())
                        }
                        if (k == "extended_si") {
                            var p = $("#right-side-tabs .info-container .extended-list-data");
                            var t = (n.replace(/aria-expanded="true"/gi, 'aria-expanded="false"').replace(/collapse in/gi, "collapse"));
                            opened_tabs = $("#extended_sysinfo .panel .in").map(function() {
                                t = (t.replace('data-toggle="collapse" href="#' + this.id + '" aria-expanded="false" aria-controls="' + this.id + '"', 'data-toggle="collapse" href="#' + this.id + '" aria-expanded="true" aria-controls="' + this.id + '"'));
                                t = (t.replace('div id="' + this.id + '" class="panel-collapse collapse"', 'div id="' + this.id + '" class="panel-collapse collapse in"'))
                            }).promise().done(function() {
                                p.html('<div style="height: 11px"></div>' + t);
                                var w = $("#right-side-tabs .extended-list-data");
                                w.find("div.panel-heading:not(#status_services-status):not(#status-virtual-server):not(#acl_logins-acl):not(#sysinfo-virtual-server):not(#quota-virtual-server):not(#bw-virtual-server)").parent().remove();
                                $.each(w.find("div.panel-heading .panel-title"), function() {
                                    var y = $(this).find("a"),
                                        x = $(this).find("a").attr("aria-expanded") != "true" ? 1 : 0,
                                        z = (x ? "+" : "—");
                                    $(this).append('<span class="pull-right panel-title-status' + (x ? " margined-right-2" : "") + '">' + z + "</span>");
                                    y.text(upperFirst($.trim(y.text())))
                                });
                                w.find(".panel.panel-default").detach().appendTo("#right-side-tabs #extended_sysinfo");
                                w.find("div.panel-body").find("img").remove();
                                w.find("a").attr("target", "page");
                                w.find('input[type="submit"]').addClass("btn btn-default btn-xs");
                                w.find('input[type="submit"]').parents("form").attr("formtarget", "page");
                                w.find('button[type="button"].ui_submit').addClass("btn btn-default btn-xs");
                                w.find('button[type="button"].ui_submit').parents("form").attr("formtarget", "page");
            ...
            if (is_module("shell") || C) {
                ...
                $.ajax({
                    ...
                    success: function(a) {
                        ...
                        q.text(F ? F : $.url(newPwd).segment(-1)).attr("data-pwd", F ? F : newPwd).attr("title", F ? F : newPwd);
                        ported_shell_size();
                        ported_shell_clear_cmd(cmdInput);
                        g.scrollTop(g[0].scrollHeight);
                        setTimeout(function() {
