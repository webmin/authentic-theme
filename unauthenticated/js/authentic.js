...
function f__l__filter_r() {
	console.log("Resetting navigation menu filters");
	...
	t__wi_p.$("aside, .visible-xs.mobile-menu-toggler").attr("style", "z-index: 10; overflow: visible; transform: translate(260px, 0px);" + a);
...
function f__c__filter_r() {
	console.log("Resetting content page filters");
...
function ___f__tw() {
	if ($("body").attr("class") && $("body").attr("class").indexOf("filemin") > -1) {
		...
		$("body").on("click", ".breadcrumb li > a, .dropdown-menu.at-filemin-favorites-dropdown > li:not(.data-context-bookmarks) > a:not(.no_effect), #DataTables_Table_0 label > a.filemin-follow-file", function (c) {
			c.preventDefault();
			c.stopPropagation();
...
if (t___wi.location == t__wi_p.location) {
	...
	if (t__wi_p.$___________initial === 1) {
		console.log("Welcome to Authentic Theme 17.54 https://github.com/qooob/authentic-theme")
	}
	$.ajax({
		type: "GET",
				...
				success: function (c) {
					$.each(c, function (h, l) {
						...
						if (h == "csf_title" || h == "csf_remote_version" || h == "authentic_remote_version" || h == "package_message") {
							...
							if ((typeof settings_sysinfo_theme_updates != "undefined" && settings_sysinfo_theme_updates) || (typeof settings_sysinfo_theme_updates == "undefined" && !$t___license_vm && !$t___license_cm)) {
	...
	function __f__get_fs(d) {
		...
		var c = $('#list_form > input[type="hidden"][name="path"]').val().replace(/\/$/g, "") + "/" + a.find("td:nth-child(3) a").text(),
			b = d;
		b.html('<span class="cspinner" style="margin-top: -11px; margin-left: 14px;"><span class="cspinner-icon"></span></span>');
		$.ajax({
			type: "POST",
	...
	function __f__c__m() {
		...
		a.contextMenu({
			...
			menuSelectorTriggered: function (d, b) {
				...
				if (!d.parents("tr").find('img[src$="inode-directory.png"]').length && !d.parents("tr").find('img[src$="inode-mount-point.png"]').length) {
					$("a[data-context-calculate-folder-size]").addClass("hidden").parent("li").addClass("hidden")
				} else {
					$("a[data-context-calculate-folder-size]").removeClass("hidden").parent("li").removeClass("hidden")
				}
		...
		_f__table = $("#list_form > table").dataTable({
					...
					if (g.length) {
						if (g.attr("symlink-title") != 1) {
							...
							$.ajax({
								...
								dataType: "text",
								success: function (s) {
									g.attr("data-title", lang("theme_xhred_filemin_title_symlink_target") + '<br><code class="text-nowrap">' + s + "</code>").attr("symlink-title", "1").data("html", true).data("placement", "right");
									g.is(":hover") ? g.tooltip("show") : g.tooltip();
			...
			initComplete: function (b) {
				...
				$("body").on("keydown", function (c) {
					...
					if ((h == 46 || h == 113 || h == 114 || h == 115 || h == 116 || h == 117 || h == 119) && !$(".ui_checked_columns input:checked").length && !$(".modal.in").length && !$("input").is(":focus") && !$("#list_form table tbody tr.m-active").length && (!c.shiftKey || (h == 116 && c.shiftKey) || (h == 115 && !c.shiftKey))) {
						c.preventDefault();
						c.stopPropagation();
					...
					if (!$(".modal.in").length && (h == 115 || h == 116 || h == 118 || String.fromCharCode(h).toLowerCase() == "c" || String.fromCharCode(h).toLowerCase() == "v" || String.fromCharCode(h).toLowerCase() == "x")) {
			...
			$.ajax({
				...
				success: function (m) {
					...
					a.on("click keyup change", function (c) {
						...
						t__wi_p.$("aside, .visible-xs.mobile-menu-toggler").attr("style", "z-index: 10; overflow: visible; transform: translate(260px, 0px);" + d);
						$('code[data-name="' + $(this).attr("name") + '"]').text($(this).val());
						settings_update()
					});
					a.each(function () {
	...
	if ($current_page_full && $current_page_full.indexOf("/sysinfo.cgi") > -1 && __num()) {
		setTimeout(function () {
			...
			if (g.text().toLowerCase() == "nan") {
				...
				$_charts_virt_percent_is_nan = 1
			} else {
				$_charts_virt_percent_is_nan = 0
			}
			...
			if (f.text().toLowerCase() == "nan") {
				b.remove();
				if ($_charts_virt_percent_is_nan) {
					c.find(".col-sm-4").removeClass("col-sm-4 col-xs-12").addClass("col-sm-6 col-xs-12")
				} else {
					c.find(".col-sm-3").removeClass("col-sm-3 col-xs-6").addClass("col-sm-4 col-xs-12")
				}
			}
