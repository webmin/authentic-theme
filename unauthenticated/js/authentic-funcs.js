...
function select_mode(e) {
    var t = $("aside form").length;
    for (i = 0; i < document.forms[0 + t].mode.length; i++) document.forms[0 + t].mode[i].checked = document.forms[0 + t].mode[i].value == e
}

function theme_select_all_link(e, t) {
    var a = $("aside form").length,
        n = document.forms[e + a][t];
    if (n)
        for (n.checked = !0, i = 0; i < n.length; i++) n[i].disabled || (n[i].checked = !0)
}
...
function theme_select_invert_link(e, t) {
    var a = $("aside form").length,
        n = document.forms[e + a][t];
    if (n)
        for (n.checked = !n.checked, i = 0; i < n.length; i++) n[i].disabled || (n[i].checked = !n[i].checked)
}
...
function get_pjax_event_end_funcs(e) {
    if (page_init(), v___module === v___module_file_manager && page.handle.content.preloader(), e || setTimeout(function() {
            ...
            var e = v___page_container.find("form:visible:first").find('input:visible:not([readonly]):not([disabled]):not([type="radio"]):not([type="checkbox"]):not([type="submit"]), textarea:visible:not([readonly]):not([disabled])');
            e.length && !e.parents(".ui_buttons_table").length && e[0].focus()
        }, 200), v___available_navigation) {
        var t = navigation_trigger(v___location.href, 1).replace(v___location_origin + v___location_prefix, "");
        "" == t || "/" == t || "/index.cgi" == t || unbuffered_header_processor_allow(v___location.href) || "csf" === v___module || "config.cgi" === v___location_file || (set_server_tmp_var_timeout = setTimeout(function() {
...
function page_sysinfo_sensor_update_label() {
    ...
    $("body").attr("class") && $("body").attr("class").indexOf(v___module_file_manager) > -1 || $.each($(".ui_checked_columns"), function(e, t) {
        !$(t).find("a[href]") || $("body").hasClass("servers") || $(this).hasClass("selectable") || $(t).addClass("cursor-pointer").find("td").addClass("cursor-pointer").find("label").addClass("cursor-pointer").find("tt").addClass("cursor-pointer"), $(t).find("td:not(.selectable)").click(function(e) {
            if (!$(e.target).is(".awobject, .iawobject, .lawobject, .awcheckbox, .awradio") && $(t).find("a[href]") && !$("body").hasClass("servers")) {
                ...
                !i || $(this).find("a").attr("href") !== $(i).attr("href") && void 0 !== $(this).find("a").attr("href") || $(e.target).is("select, input, .awobject, .iawobject, .lawobject, .awcheckbox, .awradio") || 0 !== $(this).parent("tr").find('a[href*="download.cgi"]').length || ($(i).attr("target") ? (e.preventDefault(), !a && window.open($(i).attr("href"), $(i).attr("target"))) : e.originalEvent && (e.preventDefault(), get_pjax_content($(i).attr("href")))), a && get_pjax_content($(i).attr("href"))
            }
