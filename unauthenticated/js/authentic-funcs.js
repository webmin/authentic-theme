    ...
    if (v___available_session && null != t && void 0 !== t.responseText) {
        ...
        $(a[0].attributes).each(function() {
            ...
            if (get_pjax_event_end_funcs(1), $.each($(".container-fluid img"), function() {
                ...
                }), Test.strContains(t.responseText, "theme_post_save")) {
                var i = t.responseText.match(/theme_post_save(.*)$/gm);
                i && navigation_update(i[0].split("=")[1])
            }
            if (e) {
                var a = $(e.relatedTarget),
                    n = a.attr("href"),
                    s = a.parent().is("[data-linked], .favorites-dcontainer, .menu-exclude.ui-sortable-handle");
                (s && (Test.strContains(n, "mysql") || Test.strContains(n, "postgresql")) || Test.strContains(v___module, "mysql") || Test.strContains(v___module, "postgresql")) && f__ex__mysql_runner(), (s && Test.strContains(n, "csf") || Test.strContains(v___module, "csf")) && csf_init()
            }
                ...
                $.each(e, function(e, i) {
                    if ("cpu_percent" != e && "mem_percent" != e && "virt_percent" != e && "disk_percent" != e || (Test.numeric(i) ? localStorage.setItem(v___server_hostname + "-sysinfo_" + e + "_stats", i) : localStorage.setItem(v___server_hostname + "-sysinfo_" + e + "_stats", "")), t) {
                        ...
                        if ($(t).find('.piechart[data-charts="sysinfo_' + e + '"]').length) {
                            ...
                            _.length && _.data("easyPieChart").update(i)
                        }
                    ...
                    "cpu_percent" != e && "mem_percent" != e && "virt_percent" != e && "disk_percent" != e || null !== localStorage.getItem(v___server_hostname + "-sysinfo_" + e + "_seen") && i >= 85 && localStorage.getItem(v___server_hostname + "-sysinfo_" + e + "_seen"), setTimeout(function() {
                        ...
                        if ("cpu_temperature" == e) {
                            ...
                            $.each($(i).filter(".badge-cpustatus"), function(e, t) {
                                ...
                                $(this).text().indexOf("°C") > -1 || (n = a <= 86 ? "--bg-info" : a > 86 && a <= 140 ? "--bg-success" : a > 140 && a <= 176 ? "bg-warning" : a > 176 ? "bg-danger" : ""), "bg-warning" != n && "bg-danger" != n || (_ = 1), $("#right-side-tabs .info-container .badge-drivestatus.badge-cpustatus").length || $('strong[data-stats="cpu"] + br[data-stats="cpu"]').after(i.replace(/<br>|&nbsp;/gi, "")), $($("#right-side-tabs .info-container .badge-drivestatus.badge-cpustatus")[e]).html($(this).text()).removeClass(function(e, t) {
                                    return (t.match(/(^|\s)bg-\S+/g) || []).join(" ")
                                }).addClass(n)
                            }).promise().done(function() {
                                _ ? $('.info-container [data-stats="cpu"]').removeClass("hidden") : $('.info-container [data-stats="cpu"]').addClass("hidden")
                            })
                        }
                        if ("hdd_temperature" == e) {
                            ...
                            $.each($(i), function(e, t) {
                                ...
                                r ? $('.info-container [data-stats="drive"]').removeClass("hidden") : $('.info-container [data-stats="drive"]').addClass("hidden")
                            })
                        }
...
function page_render(e) {
    ...
    if (Core.curModuleFile("virtual-server", "list_scripts.cgi") && ($('td > a > img[src*="images/staroff.gif"]').each(function(e, t) {
            $(this);
    ...
    if ($(".help_popup").on("click", function(e) {
        ...
        $.each($(".file_chooser_button"), function() {
            ...
            "." == $(this).text().substr(-1) && $(this).text($(this).text().substr(0, $(this).text().length - 1)), "." == $(this).text().substr(-1) && $(this).text($(this).text().substr(0, $(this).text().length - 1)), "." == $(this).text().substr(-1) && $(this).text($(this).text().substr(0, $(this).text().length - 1))
...
function unbuffered_header_processor(e, t) {
    if (t) {
        ...
        a.find('input[name="' + n + '"]') && a.find('input[name="' + n + '"]').val() == (s || _) || $("<input>").attr({
            type: "hidden",
            name: n,
            value: s || _
        }).appendTo(a);
    ...
    NProgress.start(), set_onbeforeunload_status(1, 1), progressive_request = $.ajax({
        ...
        data: !!t && $formData,
        complete: function(e) {
            $('div[data-dcontainer="1"]').html($(e.responseText).filter('div[data-dcontainer="1"]').html()), get_pjax_event_end(!1, e), unbuffered_header_processor_allow_scroll() && $(".__page").scrollTop($(".__page")[0].scrollHeight), $(".__page").removeClass("progressing"), NProgress.done(), set_onbeforeunload_status(0, 1)
        }
    })
}
