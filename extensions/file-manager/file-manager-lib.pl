...
{
return $userconfig{'config_portable_module_filemanager_show_dot_files'} ne
    'false' &&
    get_user_config('config_portable_module_filemanager_show_dot_files') ne
    'false';
}

sub get_user_config_datetime_from_locale
{
return $userconfig{'config_portable_module_filemanager_datetime_from_locale'}
    eq 'true' ||
    get_user_config('config_portable_module_filemanager_datetime_from_locale')
    eq 'true';
}
...
my $start = sub {
	...
	$start =
	    "<div class=\"dataTables_paginate paging_simple_numbers spaginates$invisible\">";
...
sub fm_normalize_path_name
{
my ($name, $under_dir) = @_;
return undef if (!defined($name));
$name =~ s/^\Q$under_dir\E(?:\/|$)// if ($under_dir);
$name =~ s/^\/+//;
return $name;
}

sub fm_path_is_allowed
{
my ($file, $under_dir) = @_;
...
sub fm_checked_path_under
{
my ($under_dir, $name) = @_;
$name = fm_normalize_path_name($name, $under_dir);
return undef if (fm_path_name_invalid($name));
my $file = &simplify_path("$under_dir/$name");
return undef if (!defined($file));
return fm_path_is_allowed($file, $under_dir) ? $file : undef;
}

sub fm_checked_cwd_path
{
return fm_checked_path_under($cwd, $_[0]);
}

sub fm_not_allowed_error
{
my ($name, $file) = @_;
$file ||= defined($name) ? "$cwd/$name" : $cwd;
return &text('notallowed', &html_escape($file),
	&html_escape(join(" , ", @allowed_paths)));
}

sub fm_checked_cwd_path_or_error
{
my ($name) = @_;
my $file = fm_checked_cwd_path($name);
print_error(fm_not_allowed_error($name, $file)) if (!$file);
return $file;
}
...
if (length($grep) || length($replace)) {
	if (length($grep)) {
		...
		fdo {
			...
			if ((!$regex && index($text, $grep) != -1) ||
				($regex && $text =~ /$grep/))
			{
				if (
					!grep(/^\Q$results[$file]\E$/,
						@replaces)
				    )
				{
...
my $tuconfig_per_paginate = get_user_config(
	'config_portable_module_filemanager_records_for_server_pagination');
my $max_allowed =
    int($tuconfig_per_paginate) || int($userconfig{'max_allowed'});
...
if ($userconfig{'columns'} =~ /attributes/ && get_attr_status()) {
	...
	%attributes =
	    map { $_->[1] => ('<span data-attributes>'.$_->[0].'</span>') }
	    @attributesArr;
	}
...
push(@allowed_for_edit,
	map { (my $__ = $_) =~ s/-x-/-/; $__ ne $_ ? $__ : () }
	    @allowed_for_edit);
...
if ($info_files eq 1 && $info_folders eq 1) {
	$info_total =
	    ('filemanager_global_info'.$total_with_pagination.'_total1');
	}
elsif ($info_files ne 1 && $info_folders eq 1) {
	$info_total =
	    ('filemanager_global_info'.$total_with_pagination.'_total2');
	}
elsif ($info_files eq 1 && $info_folders ne 1) {
	$info_total =
	    ('filemanager_global_info'.$total_with_pagination.'_total3');
	}
else {
	$info_total =
	    ('filemanager_global_info'.$total_with_pagination.'_total4');
	}
$list_data{'total'} = "<div class='total'>".
    (
	$query ? (trim($text{'filemanager_global_search_results'}).": ")
	: ($server_pagination
		? (trim($text{'filemanager_global_paginated_results'}).": ")
		: undef)
    ).
    "".
    (
	text(
		nice_number($info_total, ","),
...
for (my $count = 1 ; $count <= $totals_spliced ; $count++) {
	...
	if ($list[$count - 1][15] == 1) {
		...
		if (
			...
			($type_archive =~ /application-zip/ ||
				...
				$type_archive =~
				/application-x-deb|debian\.binary-package/ ||
				$type_archive =~
	...
	my @row_data = (
		"<a href='$href' data-filemin-link=\"$hlink\"".
		    (
			$query ? " data-filemin-flink=\"$hlink_path\"" : undef
		    ).
		    "><img src=\"$img\"></a>",
		"<a href=\"$href\" data-filemin-link=\"$hlink\"".
		    (
	...
	if ($userconfig{'columns'} =~ /last_mod_time/) {
		my $access_time = POSIX::strftime('%Y/%m/%d - %T',
			localtime($list[$count - 1][9]));
		my $mod_time = POSIX::strftime('%Y/%m/%d - %T',
			localtime($list[$count - 1][10]));
		my $change_time = POSIX::strftime('%Y/%m/%d - %T',
			localtime($list[$count - 1][11]));
	...
	$list_data{'rows'} .=
	    &ui_checked_columns_row(\@row_data, \@td_tags, "name", $vlink);
	}
...
$list_data{'error_fatal'} =
    (length $in{'error_fatal'} ? $in{'error_fatal'} : undef);
